...
# Set page configuration (only once at the start of the script)
st.set_page_config(page_title="eTrendTracker", layout="wide")
...
with streamlit_analytics.track(save_to_json="dashboard.json"):

    if selection == "Home":
        # Add your code for the "Home" page here
        layout="wide",
        initial_sidebar_state="expanded",  # You can set this to "collapsed" if you want the sidebar initially collapsed
        
        # Create a container with two columns
        col1, col2, col3, col4 = st.columns([2, 7, 1, 5])

        # Column 1: Header, Subtitle, and Call to Action Button
        with col1:
            pass

        with col2:
            st.markdown("#### eTrendTracker")
            st.text("")
            st.text("")
            st.markdown("# Your One-Stop E-commerce Analytics Solution")
            st.markdown("""
                        ##### eTrendTracker simplifies the way you analyze user behavior, discover trends, and
            ...
            if st.button("Go to Dashboard"):
                st.experimental_set_query_params(page=PAGES["Dashboard"])


        # Column 2: Image
        with col3:
            st.image("landing.png", caption="Landing Image", width=400)  # Adjust the width as needed

        with col4:
            pass
    elif selection == "Dashboard":
        st.title("eTrendTracker Dashboard")
        # Load the dataset
        @st.cache_data
        def load_data():
